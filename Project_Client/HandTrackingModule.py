    ...
    def action_estimation(self, img):
        ...
        if self.results.multi_hand_landmarks:
            for res in self.results.multi_hand_landmarks:
                ...
                cv2.putText(img, f'{self.this_action.upper()}', org=(int(res.landmark[0].x * img.shape[1]), int(res.landmark[0].y * img.shape[0] + 20)), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=1, color=(255, 255, 255), thickness=2)
